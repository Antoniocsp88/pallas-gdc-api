...
def _parse_filters(filters: Optional[str]) -> Optional[str]:
    ...
    if not filters:
        return None
    try:
        # If it was URL-encoded by Swagger/input, decode first
...
def _gdc_get(path: str, params: Optional[Dict[str, Any]] = None) -> JSONResponse:
    ...
    try:
        ...
        # Surface GDC JSON errors if present
        try:
...
def _download_into_tar(ids: List[str]) -> bytes:
    ...
    with tarfile.open(mode="w", fileobj=buf) as tf:
        for fid in ids:
            ...
            # write each response fully into memory chunk-by-chunk
...
def _ids_from_query(ids: Optional[List[str]] = None, ids_csv: Optional[str] = None) -> List[str]:
    ...
    collected: List[str] = []
    if ids:
        # If Swagger sends a single comma-separated string as one list item, split it.
...
def list_projects(
    ...
    filters: Optional[str] = Query(None, description='JSON filters or URL-encoded JSON (e.g., {"op":"in","content":{"field":"project_id","value":["TCGA-BRCA"]}})'),
    ...
    if filters:
        ...
        # IMPORTANT: /projects index uses field 'project_id'
...
def get_manifest(file_id: str = Path(..., description="Single file_id for which to request a manifest")):
    try:
        r = SESSION.get(GDC_MANIFEST, params={"ids": file_id}, timeout=TIMEOUT)
        r.raise_for_status()
        # GDC returns plain text manifest
        content = r.text
...
def get_data_file(file_id: str = Path(..., description="GDC file_id to download")):
    ...
    headers = {
        ...
        # Preserve original content type if present; GDC usually serves application/gzip for MAF.gz, etc.
...
@app.get(
    "/data",
    responses={200: {"description": "Bundle download (.tar)"}},
)
def get_data_bundle(
...
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    # Reuse FastAPI's renderer but force 400
